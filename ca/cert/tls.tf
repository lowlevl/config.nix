resource "tls_private_key" "private_key" {
  algorithm = var.from.output.key_algorithm
  rsa_bits  = var.bits
}
...
resource "tls_cert_request" "cert_request" {
  ...
  subject {
    organization = var.from.output.subject.0.organization
    common_name  = var.cn
  }
}
...
resource "tls_locally_signed_cert" "cert" {
  ...
  ca_private_key_pem = var.from.output.private_key_pem
  ca_cert_pem        = var.from.output.cert_pem
